...
**无角色人格**：主会话是唯一控制平面，负责用户交互、人工审批和阶段推进；每个普通 `general-purpose` subagent 不携带任何既定身份，只接受一份经过批准、权限最小化、可验证的**任务合同**，执行后交回证据。曾讨论过的按角色分工的协作方式已废弃，改为按阶段与合同分工（见 `docs/decisions/ADR-005.md`）。
