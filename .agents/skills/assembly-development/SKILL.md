...
5. **上下文恢复**：先读取当前身份对应的进度文件：项目主会话读 `Outline_Notes.md`，模块会话读 `Mxx_Outline_Notes.md`，任务会话读 `<TASK_ID>_Outline_Notes.md`。只按文件中的 `contract_sha256`、`task_revision`、`test_revision`、`evidence_path` 和 `next_action` 读取必要的上级合同、当前测试和证据；不要先扫描整个项目。进度文件缺失、过期或哈希不匹配时暂停并报告。
