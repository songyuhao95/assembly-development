...
`node scripts/self-test.mjs` 检查：
- `~/.claude/skills/grill-me`（主会话澄清用）
- `~/.claude/skills/web-design-guidelines`（前端检查）
- `~/.claude/skills/design-taste-frontend`（前端审美）
- `~/.claude/skills/ui-ux-pro-max`（UI/UX 质量）
...
3. **用户批准后**才复制最小依赖闭包（SKILL.md + 其引用的 references/scripts/data）到 `~/.claude/skills` 或本项目 `.claude/skills`。
4. 记录 `third-party/sources.lock.json`（repo、commit、文件清单、sha256、许可证、审查人、时间）。
5. 未经批准绝不执行上游脚本（skill.sh、npx、npm install 等）。

## 运行期使用

- grill-me 由**主会话**运行（subagent 不能直接与用户交互）。
